if (location.pathname !== "../touch.html" && /Mobi/.test(navigator.userAgent)) {
  location.replace('../touch.html');

}
let camera, scene, renderer, container, eingabe, canvasDown, currentCanvasRow, currentCanvasCol, ctx, c, difficulty,
  score, fruit, beginningBlockNumber, gameLost, direction, geometry, material, material2, materialsnake,
  materialsnakehead, geometrysnake, texturesnake, texturesnakehead, edges, edges2, edges3, edges4, mesh, meshes,
  geometry2, mesh2, geometry3, mesh3, geometry4, mesh4, texture, helper,
  sun, camerasettings, camerasettings2, draw;
...
function init() {
  ...
  texture = new THREE.TextureLoader().load('images/seamless space.png');
  materialSB = new THREE.MeshBasicMaterial({
    map: texture
  });
  ...
  loader.load('images/texture_sun_prev2.jpg',
    function(texture) {
  ...
  texture = new THREE.TextureLoader().load('images/bricks2.jpg');
  ...
  texturesnake = new THREE.TextureLoader().load('images/schlange.jpg');
  texturesnakehead = new THREE.TextureLoader().load('images/kopf.jpg');
  materialsnake = new THREE.MeshBasicMaterial({
    map: texturesnake
  });
  materialsnakehead = new THREE.MeshBasicMaterial({
    map: texturesnakehead
  });
  ...
  var directionalLight = new THREE.DirectionalLight(0x000000, 2);
...
function animate() {
  ...
  if (doUpdate) {
    ...
    if (gameLost !== -1) {
      ...
      if (headBB.intersectsBox(northBB) || headBB.intersectsBox(southBB) || headBB.intersectsBox(eastBB) || headBB.intersectsBox(westBB)) {
        gameLost = 0;
      }
